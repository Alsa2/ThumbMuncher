...
2. **Armed, waiting for spin**: relay on, choke closed, throttle at the FRAM/runtime `start_us` setting
3. **Spin detected**: when RPM crosses `RPM_DETECT_THRESHOLD`, choke opens and throttle keeps the same configured `start_us` for `start_hold_ms`.
...
Fallback trigger: a raw ESC command of `SERVO_TEST_RAWCOMMAND_VALUE` (`-8192`) on the resolved runtime ESC index also requests the same test. The runtime index is `board_id - 1` for board IDs 1..32; otherwise it falls back to `DRONECAN_ESC_INDEX`. The engine state machine still rejects it unless it is safe.
...
- `ENGINE_PRIMING_AFTER_SPIN`: faster LED/buzzer pulse while the configured startup PWM is held after first RPM.
...
- `ENGINE_SERVO_TEST`: fast synchronized blink/beep while the legacy relay-powered servo sweep is active.
- `ENGINE_MANUAL_PWM_TEST`: relay/servo rail is powered for the debug direct-PWM popup; starter stays off and the exact requested throttle PWM is commanded.
