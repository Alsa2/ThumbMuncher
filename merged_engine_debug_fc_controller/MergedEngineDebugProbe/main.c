...
static bool send_auto_endpoint(bool endpoint_is_max, bool enable, float target_rpm, uint16_t rate_us_per_s, bool global)
{
    uint8_t data[8] = {0};
    ...
    return tx_frame(CUSTOM_CAN_ID_AUTO_ENDPOINT, data, 8u);
}
...
static void handle_line(char *line)
{
    ...
    if (strcmp(cmd, "INFO") == 0) {
        printf("INFO bridge=rp2040_usb_can_debug_probe protocol=custom_can_v2 bitrate=%u selected=%lu commands=PING,INFO,PROBE,SELECT,CLEAR_SELECT,CMD,ARM,THROTTLE,PID,FF0,FF100,THRESH,RATE,IDENTIFY,BEEP,SERVO_TEST,AUTO0,AUTO100,AUTO_STOP,SETID,PANIC_ALL\r\n",
               CAN_BITRATE_HZ,
               (unsigned long)g_selected_id);
        return;
    }
    ...
    if (strcmp(cmd, "ARM") == 0) {
        ...
        if (sscanf(args, "%d", &armed) == 1) {
            ...
            if (!g_armed) {
                g_throttle_pct = 0.0f;
                (void)send_auto_endpoint(false, false, 0.0f, 0u, false);
            }
    ...
    if (strcmp(cmd, "AUTO0") == 0 || strcmp(cmd, "AUTO100") == 0) {
        float rpm = 0.0f;
        unsigned int rate = 8u;
        if (sscanf(args, "%f %u", &rpm, &rate) >= 1 && rate <= 65535u) {
            const bool is_max = strcmp(cmd, "AUTO100") == 0;
            g_throttle_pct = is_max ? 100.0f : 0.0f;
            print_ok_or_busy(cmd, send_auto_endpoint(is_max, true, rpm, (uint16_t)rate, false));
            (void)send_command_frame();
        } else {
            printf("ERR BAD_AUTO usage=%s <target_rpm> [rate_us_per_s]\r\n", cmd);
        }
        return;
    }

    if (strcmp(cmd, "AUTO_STOP") == 0) {
        print_ok_or_busy("AUTO_STOP", send_action(CUSTOM_CAN_ACTION_STOP_AUTO, false));
        return;
    }
...
int main(void)
{
    ...
    printf("INFO commands=PING,INFO,PROBE,SCAN,SELECT,CLEAR_SELECT,CMD,ARM,THROTTLE,PID,FF0,FF100,THRESH,RATE,IDENTIFY,BLINK,BEEP,SERVO_TEST,AUTO0,AUTO100,AUTO_STOP,SETID,PANIC_ALL\r\n");
