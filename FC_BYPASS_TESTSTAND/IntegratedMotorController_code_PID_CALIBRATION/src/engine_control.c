...
{
    if (!is_finite_local(kp) || !is_finite_local(ki) || !is_finite_local(kd) || !is_finite_local(correction_limit_us)) {
        return false;
    }
    if (kp < 0.0f || ki < 0.0f || kd < 0.0f || correction_limit_us <= 0.0f || correction_limit_us > 2000.0f) {
        return false;
    }

    g_cfg.kp_us_per_rpm = kp;
    g_cfg.ki_us_per_rpm_s = ki;
    g_cfg.kd_us_per_rpm_per_s = kd;
    g_cfg.correction_limit_us = correction_limit_us;
    reset_pid_with_current_config();
    return true;
}
