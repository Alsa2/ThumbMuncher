                             ...
                             EngineState state)
{
    ...
    case ENGINE_DISARMED:
        if (fc_alive) {
            // Command link is present but the system is not armed yet:
            ...
            // buzzer quiet so the pre-arm beep specifically means the command link is present.
    ...
    case ENGINE_PRIMING_AFTER_SPIN:
        // Active 1500 us/1 s priming pulse: faster LED + buzzer cadence.
    ...
    // If the command link disappears while armed, make that obvious even
